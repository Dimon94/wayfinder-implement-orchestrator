...
map child label，body 首行写 map 坐标。默认执行路径里，当前 owner 每完成一张票就重索引
map；每个 lane terminal 后由 `collect` 门禁立即重索引。closed 票给 Decisions-so-far 追加一行，
已知未成票缺口写进 Not yet specified。map 写「（无）」而 tracker 有 open in-scope 票即为
脱图，先归位再执行下一张。
