...
map child label，body 首行写 map 坐标。`collect` 门禁每次 wake-up 重索引 map：
...
implementation child 发现票面外活跃消费者、被推翻合同或超出单个 fresh session
安全边界的爆炸半径时：child 停止并在 final report 给出新前置票建议；父线程把
