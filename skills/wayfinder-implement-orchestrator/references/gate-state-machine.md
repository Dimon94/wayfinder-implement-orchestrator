...
| `tickets` | 已发布 tracker implementation tickets/issues | 已批准 implementation tickets 有真实 issue IDs、read-back bodies 和 dependencies；duplicate 只按当前 route truth source 判断 | Split/merge/dependency judgement、当前 source 下的 duplicate、或 partial publish |
| `dispatch` | Tracker implementation tickets/issues | Ready tickets 没有 blocking dependencies 或 mutable-resource conflicts | ticket set 模糊、files/resources 重叠、缺少 base branch |
...
| `remote-review` | GitHub PR 或 GitLab MR、CI/CD status、review-agent comments | PR/MR 链接 spec/tickets/child threads/commits/checks/risks，CI/CD 通过，且 review Agent 说 PR/MR can pass | Push/open PR/MR 权限不清晰、CI/CD 失败、valid/Unknown review rejection，或未解决的 review-Agent mistake |
...
turn 中为剩余每个 ticket 创建一个 `/implement` child，然后用一个 heartbeat 监控整个
batch。
