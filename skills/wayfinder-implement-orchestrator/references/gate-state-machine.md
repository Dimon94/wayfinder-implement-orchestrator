...
| `dispatch` | Tracker implementation tickets/issues、`frontier-lanes.md` | 已重算 ready frontier，并把 maximal safe batch 自动放入有独立 worktree/branch 的 execution lanes；存量票已补估档位与足迹，lane packet 携带档位上限；仪表盘数据层在派发完成后刷新并打开（不作为派发前置） | ticket set 模糊、冲突无法隔离、缺少 local execution authority 或 base branch |
