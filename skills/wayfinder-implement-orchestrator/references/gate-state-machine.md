...
| `collect` | 5 分钟 automation wake-up、child thread readback | 每个 child 报告 status、commit if changed、checks、dirty state、touched files；map 已重索引：修补票挂图、Decisions-so-far 与 Not yet specified 同步（`ticket-split-coverage.md`） | child blocked/off-scope 或缺少 final report |
