...
| `dispatch` | Tracker implementation tickets/issues、`frontier-lanes.md` | 已重算 ready frontier，并把 maximal safe batch 自动放入有独立 worktree/branch 的 execution lanes | ticket set 模糊、冲突无法隔离、缺少 local execution authority 或 base branch |
...
| `collect` | terminal events、lane final reports | terminal lane 报告 commit、checks、dirty state、touched files；coordinator 只读取一次并立即重算 frontier | 缺少 terminal report，或 watchdog 发现 setup/signal/tool timeout |
