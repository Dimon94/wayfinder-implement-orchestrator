...
| `dispatch` | Tracker tickets、`frontier-lanes.md` | ready frontier、maximal safe batch、lane IDs、独立 worktree/branch、runtime 和 local execution authority 已冻结；存量票已补估档位，lane packet 携带档位上限；仪表盘数据层在派发完成后刷新并打开（不作为派发前置） | ticket set、base、local authority 或冲突边界模糊 |
