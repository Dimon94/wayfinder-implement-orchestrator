...
| `tickets` | 已发布 tracker implementation tickets/issues | 已批准 implementation tickets 有真实 issue IDs、read-back bodies 和 dependencies；duplicate 只按当前 route truth source 判断；变更面普查六面各有票或 map 边界行；每票带估时档位与足迹，无 XL 且 L 有不拆理由（`ticket-split-coverage.md`）；仪表盘已生成并打开（`map-dashboard.md`） | Split/merge/dependency judgement、当前 source 下的 duplicate、或 partial publish |
| `dispatch` | Tracker tickets、`frontier-lanes.md` | ready frontier、maximal safe batch、lane IDs、独立 worktree/branch、runtime 和 local execution authority 已冻结；存量票已补估档位与足迹，lane packet 携带档位上限；仪表盘数据层在派发完成后刷新并打开（不作为派发前置） | ticket set、base、local authority 或冲突边界模糊 |
...
| `collect` | terminal events、lane final reports | 每条 terminal lane 已被读取一次并验证；blocked 只暂停本 lane；`estimate-log.csv` 已按 final report 追加行 | final report 缺失、commit/dirty state 不可信 |
