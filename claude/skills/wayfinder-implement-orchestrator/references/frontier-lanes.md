...
ordering、尚未满足的 dependency、同一 tracker item 写权限。文件冲突优先用票面
`足迹：` 字段判断：足迹相交的票串进同一 lane。缺少冲突证据时只串行相关项，
不把整个 frontier 降成串行。
...
1. maximal safe batch 中每张 implementation ticket 启动一个 lane；所有 lanes 按执行通道
   创建 Codex 或 Claude panes。Coordinator 不亲自执行任何 lane，只做调度和 fan-in。
